  ...
  static validate(
    ...
    if (sprites) {
      for (let i = 0; i < 256; i++) {
        ...
        if (p.some(v => v !== 0)) definedTilesCount++;
      }
      for (let i = 0; i < 256; i++) {
        const p = sprites.getSpritePixels(i, false);
        if (p.some(v => v !== 0)) definedSpritesCount++;
      }
    }
    ...
    if (audio) {
      for (let i = 0; i < 64; i++) {
        ...
        if (s && s.notes.some(n => n.note >= 0 || n.volume > 0)) definedSfxCount++;
        const p = audio.getPattern(i);
        if (p && p.rows.some(r => r.some(c => c.note >= 0))) definedPatternsCount++;
        const t = audio.getTrack(i);
        if (t && t.patterns.some(pt => pt >= 0)) definedTracksCount++;
      }
    }
