...
export function parseNoteString(str: string): { note: number; octave: number } | null {
...
export function formatNote(note: number, octave: number): string {
  if (note < 0 || note >= 12 || octave < 1) return '---';
  return `${NOTE_NAMES[note]}${octave}`;
}

export class AudioManager {
  private waveforms: Uint8Array = new Uint8Array(MAX_WAVEFORMS * WAVEFORM_SIZE); // 16 * 32 nibbles
  private sfxList: SFXData[] = [];
  ...
  setWaveform(index: number, samples: number[]) {
    const start = (index & 15) * 32;
  ...
  setSFX(id: number, data: Partial<SFXData>) {
    const sfx = this.sfxList[id & 63];
    if (!sfx) return;
    if (data.speed !== undefined) sfx.speed = Math.max(1, Math.min(64, data.speed));
    if (data.notes) {
  ...
  setPatternRow(patternId: number, row: number, channel: number, data: Partial<ChannelRow>) {
    const pat = this.patterns[patternId & 63];
    if (!pat || row < 0 || row >= PATTERN_ROWS || channel < 0 || channel >= MUSIC_CHANNELS) return;
    const cell = pat.rows[row][channel];
    if (data.note !== undefined) cell.note = data.note;
    if (data.octave !== undefined) cell.octave = data.octave;
    if (data.sfx !== undefined) cell.sfx = data.sfx;
    if (data.volume !== undefined) cell.volume = data.volume;
  }
  ...
  setTrack(id: number, data: Partial<TrackData>) {
    const track = this.tracks[id & 63];
    if (!track) return;
    if (data.tempo !== undefined) track.tempo = data.tempo;
    if (data.speed !== undefined) track.speed = data.speed;
    if (data.patterns) {
  ...
  loadFromWavesLine(line: string) {
    const match = line.match(/--\s*(\d+):([0-9a-fA-F]+)/);
    if (!match) return;
    const id = parseInt(match[1], 10);
    const hex = match[2];
    if (id < 0 || id >= MAX_WAVEFORMS) return;
    const start = id * 32;
  ...
  loadRawWaveforms(buffer: Uint8Array) {
