...
import { CartridgeMetadata, ChunkType, CartridgeValidationResult } from './types.js';
...
`;
  ...
  toText(): string {
    ...
    // 1. Metadata Header
    if (this.metadata.title) parts.push(`-- title:  ${this.metadata.title}`);
    if (this.metadata.author) parts.push(`-- author: ${this.metadata.author}`);
    if (this.metadata.desc) parts.push(`-- desc:   ${this.metadata.desc}`);
    parts.push(`-- script: ${this.metadata.script || 'lua'}`);
    if (this.metadata.input) parts.push(`-- input:  ${this.metadata.input}`);
    if (this.metadata.saveid) parts.push(`-- saveid: ${this.metadata.saveid}`);
    parts.push('');
  ...
  toBinary(): Uint8Array {
    ...
    const addChunk = (type: ChunkType, bank: number, data: Uint8Array) => {
   ...
   */
  loadFromBinary(buffer: Uint8Array) {
    let offset = 0;
    while (offset + 4 <= buffer.length) {
      const b0 = buffer[offset];
      const type = (b0 & 0x1f) as ChunkType;
