...
describe('DeepSeek Harness TIC-80 Tools Suite', () => {
  ...
  test('registers all 14 tools', () => {
    ...
    const expected = [
      ...
      'tic80_run', 'tic80_export', 'tic80_studio_status', 'tic80_cli'
    ];
...
`;
    ...
    // First write
    const res1: any = await setCodeTool.execute({ code: payloadWithHeader }, {} as any);
    ...
    const res2: any = await setCodeTool.execute({ code: content1 }, {} as any);
  ...
  test('tic80_cli executes command and captures output', async (t) => {
    ...
    const cliTool = toolMap.get('tic80_cli')!;
    const res: any = await cliTool.execute({
      command: 'help export',
      timeoutMs: 5000,
    }, {} as any);

    assert.strictEqual(res.success, true);
    assert.ok(res.output.includes('export') || res.output.includes('COMMAND'));
    assert.strictEqual(res.errors.length, 0);
  });

  test('tic80_cli captures error outputs when executing invalid commands', async (t) => {
    const exe = NativeRunner.findExecutable();
    if (!exe) {
      t.skip('tic80 native executable not available in environment');
      return;
    ...
    const cliTool = toolMap.get('tic80_cli')!;
    const res: any = await cliTool.execute({
      command: 'load non_existent_file_123.tic',
      timeoutMs: 5000,
    }, {} as any);

    assert.ok(res.errors.length > 0 || res.output.includes('not found') || res.output.includes('error'));
  });

  test('tic80_cli executes export command and detects output files', async (t) => {
    const exe = NativeRunner.findExecutable();
    if (!exe) {
      t.skip('tic80 native executable not available in environment');
      return;
    ...
    const cliTool = toolMap.get('tic80_cli')!;
    const testExportName = `cli_test_sprites_${Date.now() % 10000}.png`;
    const res: any = await cliTool.execute({
      command: `export sprites ${testExportName}`,
      timeoutMs: 5000,
    }, {} as any);

    assert.strictEqual(res.success, true);
    assert.ok(res.exportedFiles.some((f: string) => f.includes(testExportName)));

    // Cleanup generated export file
    const generatedPath = path.resolve(process.cwd(), testExportName);
