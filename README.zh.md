...
### ⚡ 专为极速原型打造 (Rapid Prototyping)

- 🎯 **开箱即用的极速游戏原型**：专为玩法创意与游戏机制验证设计，输入自然语言需求，秒级生成包含代码、精灵、地图与音效的完整可玩 Demo。
- ⚙️ **顶部配置驱动手感微调**：大模型生成代码时严格遵循工程规范，所有核心物理与玩法变量（如 `jump` 跳跃高度、`gravity` 重力、`speed` 移速、`friction` 摩擦力等）均统一提取在**代码脚本最顶部**（`cfg = { ... }`）。无需在复杂的逻辑代码深处翻找，轻松修改并即时调出理想的跳跃手感与操作反馈！
- 🗺️ **所见即所得的关卡设计（WYSIWYG）**：告别在代码中硬编码巨型关卡二维数组，直接利用 TIC-80 内置的 **Map Editor（地图编辑器，快捷键 `F3`）** 可视化绘制关卡，配合 Sprite Flags（图块碰撞标记）与地图实体解析，真正实现关卡搭建“所见即所得”。
